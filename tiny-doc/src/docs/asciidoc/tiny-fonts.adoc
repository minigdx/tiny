...
Use the `tiny-cli add` command with the `--font`, `--size`, and `--chars` options to register a PNG as a font.
...
- `--size` specifies the character cell size in pixels. Use `WxH` for rectangular cells (e.g., `6x12`) or a single number for square cells (e.g., `8`).
