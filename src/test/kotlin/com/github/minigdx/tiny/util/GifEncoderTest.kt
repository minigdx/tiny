...
class GifEncoderTest {
    ...
    @Test
    fun `writeHeader - it write the GIF header`() {
        val output = ByteArrayStream()

        encoder.writeHeader(output)

        assertEquals("GIF89a".toByteArray(), output.output)
    }


    @Test
    fun `writeHeader - it write the Logical screen header`() {
        val output = ByteArrayStream()
    ...
    @Test
    fun `writeGlobalColorTableHeader - it write the GlobalColorTable header`() {
        val output = ByteArrayStream()
