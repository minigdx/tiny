...
class LibCommand : CliktCommand(name = "lib") {
    ...
    val libs by argument(
        help =
            "The library to be added to the game. " +
                "All available libraries are available here: https://github.com/minigdx/tiny/tiny-repository-libs.",
    ).multiple(required = true)

    val update by option(help = "Update existing libraries").flag(default = false)

    override fun help(context: Context) = "Download a library for your game."
    ...
    override fun run() {
        ...
        val updatedGameParameters =
            addedLibs.fold(gameParameters) { parameter, lib ->
